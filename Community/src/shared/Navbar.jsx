...
const SisNavbar = () => {
  ...
  const fetchUsersRoles = () => {
    ...
    setIsContable(rolesFromStorage?.length !== 0 && rolesFromStorage.includes("CONTABLE"));
    setIsAdmin(rolesFromStorage?.length !== 0 && rolesFromStorage.includes("ADMIN"));
    setIsGuardia(rolesFromStorage?.length !== 0 && rolesFromStorage.includes("GUARDIA"));
    setIsResidente(rolesFromStorage?.length !== 0 && rolesFromStorage.includes("RESIDENTE"));
    setIsPropietario(rolesFromStorage?.length !== 0 && rolesFromStorage.includes("PROPIETARIO"));
    setIsTrabajador(rolesFromStorage?.length !== 0 && rolesFromStorage.includes("TRABAJADOR"));
