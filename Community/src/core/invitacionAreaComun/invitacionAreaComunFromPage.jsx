...
import { LISTAINVITADOSAREACOMUN_URL } from "../../routing/CONSTANTS";
import { GetListaAreasComunes, GetListaDuenos, GetListainvitadosareacomun, Getinvitadosareacomun, Postinvitadosareacomun, Putinvitadosareacomun, getresidencial } from "../../services/InvitacionAreaComunService";
...
const InvitacionAreaComun = () => {
    ...
    const onUserFormSubmit = (e) => {
        ...
        e.stopPropagation();
        setValidated(true);
        if (parseInt(localStorage.getItem('rol')) == 6) {
            setDueno_id(parseInt(localStorage.getItem('id')));
        }
            ...
            <Container>
                <Card className="mt-3">
                    <Card.Body>
                        ...
                        {parseInt(localStorage.getItem('rol')) == 4 || parseInt(localStorage.getItem('rol')) == 1 ? (
                            <div>
                                ...
                                <Form noValidate onSubmit={onUserFormSubmit} validated={validated}>
                                    ...
                                    <FormGroup>
                                        ...
                                        <FormSelect value={dueno_id} onChange={onChangeDueno} >
                                            ...
                                            {duenos.map((dueno) => {
                                                let recide = getresidencial(dueno.id);
                                                if(parseInt(localStorage.getItem('rol')) == 6 && recide == parseInt(localStorage.getItem('residencial_id')))
                                    ...
                                    <FormGroup>
                                        ...
                                        <FormSelect value={areascomunes_id} onChange={onChangeAreaComun} >
                                            ...
                                            {areascomunes.map((area) => {
                                                if(parseInt(localStorage.getItem('residencial_id')) == area.residencia_id)
                                ...
                                <Form noValidate onSubmit={onUserFormSubmit} validated={validated}>
                                    ...
                                    <FormGroup>
                                        ...
                                        <FormSelect value={areascomunes_id} onChange={onChangeAreaComun2} >
                                            ...
                                            {areascomunes.map((area) => {
                                                console.log(area.residencia_id == parseInt(localStorage.getItem('residencial_id')))
                                                if (area.user_id == parseInt(localStorage.getItem('id')) && area.residencia_id == parseInt(localStorage.getItem('residencial_id')))
                                                    return <option key={area.id} value={area.id}>{area.nombre}</option>
                                            })}
                                        </FormSelect>
                                    </FormGroup>
